...
1. Se seu primeiro nome for **_PEDRO_**:<br>SELECT Nome_dependente, Pnome, Unome<br>FROM DEPENDENTE JOIN FUNCIONARIO ON Fcpf = Cpf<br>WHERE ( Nome_dependente LIKE "PE%" OR Pnome LIKE "PE%" )<br>AND ( Nome_dependente LIKE "%RO" OR Pnome LIKE "%RO" )<br>AND ( CHAR_LENGTH(Nome_dependente) = CHAR_LENGTH("PEDRO") OR CHAR_LENGTH(Pnome) = CHAR_LENGTH("PEDRO") )
